...
ListNode* HashSearch(HashTable Q3Hash, int key)
{
    ...
    for(ListNode *i = Q3Hash.Table[keys].head; i != NULL; i = i->next){
...
int HashInsert(HashTable* Q3HashPtr, int key)
{
    ...
    if(HashSearch(*Q3HashPtr, key) != NULL){
        ...
        //make new linkedlistnode set it as the head :)
        temp = Q3HashPtr->Table[hash].head;
        newNode = malloc(sizeof(newNode));
        newNode->key = key;
        newNode->next = temp;
        Q3HashPtr->Table[hash].head = newNode;
